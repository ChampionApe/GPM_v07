...
class Compile:
	...
	def declareGroupsText(self,db,gs='all'):
		...
		if bool(set(metagroup.conditions)-set(self.declared.v)):
			out = "Variables \n\t"+"\n\t".join([writeGpy(s=db[var])+"\t\""+db[var].text+"\"" for var in metagroup.conditions if var not in self.declared])+"\n;"
	...
	def fixGroupText(self,db,g):
		return "\n".join([f"{writeGpy(db[k],c=v,l='.fx')} = {writeGpy(db[k],l='.l')};" for k,v in g.conditions.items()])
